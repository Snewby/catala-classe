...
personal email address and per-subscriber tracking tokens) and writes:
    ...
    js/dialogs-data.js       var DIALOGS       the A1 dialogues
    js/week-lessons-data.js  var WEEK_LESSONS  the "Week N of 25" curriculum lessons
...
def parse_week(text):
    ...
    # Some weeks trail the numbers table with a usage note (week 22: ordinal agreement).
    num_block = section(vs, "\nNumbers\n", "\nSituation Patterns\n")
    m_note = re.search(r"^Note:\s*(.+)$", num_block, re.M)
    num_note = m_note.group(1).strip() if m_note else ""
    num = parse_table(re.sub(r"^Note:.*$", "", num_block, flags=re.M))
    ...
    return {"w": w, "topic": topic, "lvl": lvl, "fl": groups, "voc": voc,
            "num": num, "nnote": num_note, "sit": sit, "md": md, "ph": ph}
...
def emit_weeks(records):
    ...
    out = [
        ...
        "// fl is empty in weeks that ship no functional-language section; nnote is an",
        "// optional usage note under the numbers table.",
